...
void do_morphset( CHAR_DATA * ch, char *argument )
{
   ...
   if( IS_NPC( ch ) )
   {
      send_to_char( "Mob's can't setmorph\n\r", ch );
      return;
   }
   ...
   switch ( ch->substate )
   {
      ...
      case SUB_MORPH_HELP:
         ...
         if( ch->substate == SUB_REPEATCMD )
            ch->dest_buf = morph;
         return;
      case SUB_MORPH_USING:
         if( !ch->dest_buf )
         {
            send_to_char( "Fatal error: report to Shaddai.\n\r", ch );
            bug( "do_morphset: sub_morph_using: NULL ch->dest_buf", 0 );
            ch->substate = SUB_NONE;
            return;
         }
         morph = ch->dest_buf;
         STRFREE( morph->isusing );
         morph->isusing = copy_buffer( ch );
         stop_editing( ch );
         ch->substate = ch->tempnum;
         if( ch->substate == SUB_REPEATCMD )
            ch->dest_buf = morph;
   ...
   if( ch->substate == SUB_REPEATCMD )
   {
      ...
      {
         send_to_char( "Setmorph mode off.\n\r", ch );
         ch->substate = SUB_NONE;
         ch->dest_buf = NULL;
   ...
   if( !str_cmp( arg1, "on" ) )
   {
      send_to_char( "Syntax: setmorph <morph> on.\n\r", ch );
      return;
   }
   ...
   if( arg1[0] == '\0' || ( arg2[0] == '\0' && ch->substate != SUB_REPEATCMD ) || !str_cmp( arg1, "?" ) )
   {
      if( ch->substate == SUB_REPEATCMD )
      {
         ...
         send_to_char( "Syntax: setmorph <morph> <field>  <value>\n\r", ch );
      send_to_char( "Syntax: setmorph save\n\r", ch );
      send_to_char( "\n\r", ch );
      ...
      send_to_char( "  dayfrom, deity, description, defpos, dex, dodge, equipment,\n\r", ch );
      send_to_char( "  favourused, gloryused, help, height, hitroll, hp, hpused, immune,\n\r", ch );
      send_to_char( "  int, str, keyword, lck, level, long, mamfin, mamfout, mana, manaused,\n\r", ch );
      ...
      send_to_char( "  obj1, obj2, obj3, useobj1, useobj2, useobj3, parry,\n\r", ch );
      ...
      send_to_char( "  tumble, unmorphother, unmorphself, weight, wis.\n\r", ch );
      send_to_char_color( "&c-------------------------------------------------\n\r", ch );
      return;
   }
   ...
   if( !str_cmp( arg2, "on" ) )
   {
      ...
      ch_printf( ch, "Setmorph mode on. (Editing %s).\n\r", morph->name );
      ...
      if( ch->pcdata )
      {
         ...
         sprintf( buf, "<&CSetmorph &W%s&w> %%i", morph->name );
   ...
   else if( !str_cmp( arg2, "pkill" ) )
   {
      ...
      {
         send_to_char( "Usage: setmorph <morph> pkill [pkill|peace|none]\n\r", ch );
         return;
      }
   }
   ...
   else if( !str_cmp( arg2, "height" ) )
   {
      ...
      {
         send_to_char( "Height range is 1 to 400 inches.\n\r", ch );
         return;
      }
      morph->height = value;
   }
   else if( !str_cmp( arg2, "weight" ) )
   {
      ...
      {
         send_to_char( "Weight range is 1 to 5000 lbs.\n\r", ch );
         return;
      }
      morph->weight = value;
   }
   ...
   {
      if( value < 0 || value > ( LEVEL_ACOLYTE ) )
      {
         send_to_char( "Level range is 0 to 400.\n\r", ch );
         return;
      }
      morph->level = value;
   }
   ...
   else if( !str_cmp( arg2, "resistant" ) || !str_cmp( arg2, "r" ) )
   {
      ...
      {
         send_to_char( "Usage: setmorph <morph> resistant <flag> [flag]...\n\r", ch );
         return;
      }
      while( argument[0] != '\0' )
      {
   ...
   else if( !str_cmp( arg2, "susceptible" ) || !str_cmp( arg2, "s" ) )
   {
      if( !argument || argument[0] == '\0' )
      {
         send_to_char( "Usage: setmorph <morph> susceptible <flag> [flag]...\n\r", ch );
         return;
      }
      while( argument[0] != '\0' )
      {
   ...
   else if( !str_cmp( arg2, "immune" ) || !str_cmp( arg2, "i" ) )
   {
      if( !argument || argument[0] == '\0' )
      {
         send_to_char( "Usage: setmorph <morph> immune <flag> [flag]...\n\r", ch );
         return;
      }
      while( argument[0] != '\0' )
      {
   ...
   else if( !str_cmp( arg2, "noresistant" ) || !str_cmp( arg2, "nr" ) )
   {
      if( !argument || argument[0] == '\0' )
      {
         send_to_char( "Usage: setmorph <morph> noresistant <flag> [flag]...\n\r", ch );
         return;
      }
      while( argument[0] != '\0' )
      {
   ...
   else if( !str_cmp( arg2, "nosusceptible" ) || !str_cmp( arg2, "ns" ) )
   {
      if( !argument || argument[0] == '\0' )
      {
         send_to_char( "Usage: setmorph <morph> nosusceptible <flag> [flag]...\n\r", ch );
         return;
      }
      while( argument[0] != '\0' )
      {
   ...
   else if( !str_cmp( arg2, "noimmune" ) || !str_cmp( arg2, "ni" ) )
   {
      if( !argument || argument[0] == '\0' )
      {
         send_to_char( "Usage: setmorph <morph> noimmune <flag> [flag]...\n\r", ch );
         return;
      }
      while( argument[0] != '\0' )
      {
   ...
   else if( !str_cmp( arg2, "affected" ) || !str_cmp( arg2, "aff" ) )
   {
      if( !argument || argument[0] == '\0' )
      {
         send_to_char( "Usage: setmorph <morph> affected <flag> [flag]...\n\r", ch );
         return;
      }
      while( argument[0] != '\0' )
      {
   ...
   else if( !str_cmp( arg2, "noaffected" ) || !str_cmp( arg2, "naff" ) )
   {
      if( !argument || argument[0] == '\0' )
      {
         send_to_char( "Usage: setmorph <morph> noaffected <flag> [flag]...\n\r", ch );
         return;
      }
      while( argument[0] != '\0' )
      {
   ...
   else if( !str_cmp( arg2, "mamfin" ) )
   {
      STRFREE( morph->mamfin );
      morph->mamfin = STRALLOC( arg3 );
   }
   else if( !str_cmp( arg2, "mamfout" ) )
   {
      STRFREE( morph->mamfout );
      morph->mamfout = STRALLOC( arg3 );
   }
   ...
   else if( !str_cmp( arg2, "equipment" ) || !str_cmp( arg2, "equip" ) )
   {
      if( arg3[0] )
      {
         STRFREE( morph->isusing );
         morph->isusing = STRALLOC( arg3 );
      }
      CHECK_SUBRESTRICTED( ch );
      if( ch->substate == SUB_REPEATCMD )
         ch->tempnum = SUB_REPEATCMD;
         ...
         ch->tempnum = SUB_NONE;
      ch->substate = SUB_MORPH_USING;
      ch->dest_buf = morph;
      start_editing( ch, morph->isusing );
      return;
   }
...
void do_morphstat( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg[0] == '\0' )
   {
      send_to_pager( "Statmorph what?\n\r", ch );
      return;
   }
   if( IS_NPC( ch ) )
   {
      send_to_char( "Mob's can't statmorph\n\r", ch );
      return;
   }
   ...
   if( argument[0] == '\0' )
   {
      ...
      pager_printf_color( ch, "  &cRaces Allowed: &w%s\n\r", race_string( morph->race ) );
      ...
      pager_printf_color( ch, "  &cUSAGES:  Mana:  &C%d  &cMove:  &C%d  &cHp:  &C%d  &cFavour:  &C%d  &cGlory:  &C%d\n\r",
                          morph->manaused, morph->moveused, morph->hpused, morph->favourused, morph->gloryused );
      ...
      pager_printf_color( ch,
                          ...
                          ( morph->objuse[1] ? "YES" : "no" ), morph->obj[2], ( morph->objuse[2] ? "YES" : "no" ) );
   ...
   else if( !str_cmp( argument, "help" ) || !str_cmp( argument, "desc" ) )
   {
      ...
      feet = morph->height / 12;
      inches = morph->height % 12;

      pager_printf_color( ch, "  &cMorph Name  : &C%-20s\n\r", morph->name );
      pager_printf_color( ch, "  &cDefault Pos : &w%d\n\r", morph->defpos );
      pager_printf_color( ch, "  &cKeywords    : &w%s\n\r", morph->key_words );
      pager_printf_color( ch, "  &cFake Height : &w%d'%d\"\n\r", feet, inches );
      pager_printf_color( ch, "  &cFake Weight : &w%d lb\n\r", morph->weight );
      ...
      send_to_pager_color( "&B[----------------------------------------------------------------------------]\n\r", ch );
      pager_printf_color( ch, "                                 &cEquipment:\n\r&w%s\n\r", morph->isusing );
      send_to_pager_color( "&B[----------------------------------------------------------------------------]\n\r", ch );
      pager_printf_color( ch, " &WIMMORTAL:\n\r&C IN: &W%s %s\n\r&COUT: &W%s %s\n\r", capitalize( morph->short_desc ),
                          morph->mamfin, capitalize( morph->short_desc ), morph->mamfout );
      send_to_pager_color( "&B[----------------------------------------------------------------------------]\n\r", ch );
   }
   else
   {
      send_to_char( "Syntax: statmorph <morph>\n\r", ch );
      send_to_char( "Syntax: statmorph <morph> <help/desc>\n\r", ch );
   }
   return;
}
...
void do_morph( CHAR_DATA * ch, MORPH_DATA * morph )
{
   ...
   ch->max_hit += ( ch->max_hit + atoi( morph->hit ) > 2000000 ) ? ( 2000000 - ch->max_hit ) : atoi( morph->hit );
   ch_morph->move = dice_parse( ch, morph->level, morph->move );
   if( ( ch->move + ch_morph->move ) > 2000000 )
      ch_morph->move = ( 2000000 - ch->move );
   ch->move += ch_morph->move;
   ch_morph->mana = dice_parse( ch, morph->level, morph->mana );
   if( ( ch->mana + ch_morph->mana ) > 2000000 )
      ch_morph->mana = ( 2000000 - ch->mana );
...
void fwrite_morph( FILE * fp, MORPH_DATA * morph )
{
   ...
   if( morph->mamfin[0] != '\0' )
      fprintf( fp, "Mamfin            %s~\n", morph->mamfin );
   if( morph->mamfout[0] != '\0' )
      fprintf( fp, "Mamfout           %s~\n", morph->mamfout );
   if( morph->mana[0] != '\0' )
      fprintf( fp, "Mana            	%s~\n", morph->mana );
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'D':
            ...
            break;
         case 'E':
            KEY( "Equipment", morph->isusing, fread_string_nohash( fp ) );
            if( !str_cmp( word, "End" ) )
               return morph;
            break;
         case 'F':
         ...
         case 'G':
            ...
            break;
         case 'H':
            KEY( "Height", morph->height, fread_number( fp ) );
         ...
         case 'L':
            ...
            break;
         case 'M':
            KEY( "Mamfin", morph->mamfin, fread_string_nohash( fp ) );
            KEY( "Mamfout", morph->mamfout, fread_string_nohash( fp ) );
         ...
         case 'V':
            ...
            break;
         case 'W':
            KEY( "Weight", morph->weight, fread_number( fp ) );
...
void free_morph( MORPH_DATA * morph )
{
   ...
   if( morph->isusing )
      DISPOSE( morph->isusing );
   if( morph->key_words )
      DISPOSE( morph->key_words );
   if( morph->long_desc )
      DISPOSE( morph->long_desc );
   if( morph->mamfin )
      DISPOSE( morph->mamfin );
   if( morph->mamfout )
      DISPOSE( morph->mamfout );
   if( morph->mana )
      DISPOSE( morph->mana );
...
void morph_defaults( MORPH_DATA * morph )
{
   ...
   morph->isusing = str_dup( "" );
   morph->key_words = str_dup( "" );
   morph->long_desc = str_dup( "" );
   morph->mamfin = str_dup( "appears in a shimmer of light." );
   morph->mamfout = str_dup( "shimmers and fades from view." );
   morph->mana = str_dup( "" );
...
void copy_morph( MORPH_DATA * morph, MORPH_DATA * temp )
{
   ...
   morph->isusing = str_dup( temp->isusing );
   morph->key_words = str_dup( temp->key_words );
   morph->long_desc = str_dup( temp->long_desc );
   morph->mamfin = str_dup( temp->mamfin );
   morph->mamfout = str_dup( temp->mamfout );
...
void do_morphcreate( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg1[0] == '\0' )
   {
      send_to_char( "Usage: makemorph <name>\n\r", ch );
      send_to_char( "Usage: makemorph <name/vnum> copy\n\r", ch );
      return;
   }
