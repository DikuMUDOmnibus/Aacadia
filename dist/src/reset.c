...
void do_reset( CHAR_DATA * ch, char *argument )
{
   ...
   if( !pArea && get_trust( ch ) > LEVEL_BUILD )
   {
      char fname[80];
...
void do_instaroom( CHAR_DATA * ch, char *argument )
{
   ...
   if( IS_NPC( ch ) || get_trust( ch ) < LEVEL_BUILD || !ch->pcdata || !ch->pcdata->area )
   {
   ...
   if( !can_rmodify( ch, pRoom ) )
      return;
   if( pRoom->area != pArea && get_trust( ch ) < LEVEL_BUILD )
   {
      ...
      switch ( pObjIndex->item_type )
      {
         ...
         case ITEM_PILL:
            if( pObjIndex->value[7] > 0 )
               olevel = UMIN( pObjIndex->value[7], MAX_LEVEL );
            else
...
void basestat( OBJ_DATA * obj )
{
   int x = 0;

   if( obj->level > 0 && obj->level < 20 )
   {
      x = number_range( 1, 4 );
   }
   else if( obj->level > 19 && obj->level < 50 )
   {
      x = number_range( 3, 6 );
   }
   else if( obj->level > 49 && obj->level < 100 )
   {
      x = number_range( 5, 9 );
   }
   else if( obj->level >= 100 && obj->level < 200 )
   {
      x = number_range( 10, 13 );
   }
   else if( obj->level >= 200 && obj->level < 300 )
   {
      x = number_range( 23, 26 );
   }
   else if( obj->level >= 300 && obj->level < 400 )
   {
      x = number_range( 36, 39 );
   }
   else if( obj->level >= 400 && obj->level < 600 )
   {
      x = number_range( 49, 52 );
   }
   else if( obj->level >= 600 )
   {
      x = 100;
   }
   obj->value[0] = x;
...
/* Inventory problem */
            ...
            if( obj->value[7] > 0 )
               obj->level = URANGE( 0, obj->value[7], LEVEL_IMMORTAL );
            else
               obj->level = URANGE( 0, obj->level, LEVEL_IMMORTAL );
            if( IS_ARM_TYPE( obj ) )
            {
               basestat( obj );
            }
      ...
      switch ( pReset->command )
      {
         ...
         case 'M':
            ...
            if( !( room = get_room_index( pReset->arg3 ) ) )
               ...
               rname = room->name;
            sprintf( pbuf, "%s (&R%d&w) &p->&w %s (&R%d&w) [%d]", mname, pReset->arg1, rname, pReset->arg3, pReset->arg2 );
            if( !room )
               mob = NULL;
         ...
         case 'E':
            ...
            if( !( obj = get_obj_index( pReset->arg1 ) ) )
               ...
               oname = obj->name;
            sprintf( pbuf, "%s (&R%d&w) &p->&w %s (%s) [%d]", oname, pReset->arg1, mname,
         ...
         case 'O':
            ...
            if( !( room = get_room_index( pReset->arg3 ) ) )
               ...
               rname = room->name;
            sprintf( pbuf, "(object) %s (&R%d&w) &p->&w %s (&R%d&w) [%d]&D\n\r", oname,
         ...
         case 'B':
         {
            ...
            switch ( pReset->arg2 & BIT_RESET_TYPE_MASK )
            {
               case BIT_RESET_DOOR:
               {
                  ...
                  sprintf( pbuf, "Exit %s%s (%d), Room %s (&R%d&w)&D", dir_name[door],
         ...
         case 'D':
         {
            ...
            sprintf( pbuf, "%s [%d] the %s%s [%d] door %s (&R%d&w)&D\n\r", ef_name,
         ...
         case 'R':
            if( !( room = get_room_index( pReset->arg1 ) ) )
               ...
               rname = room->name;
            sprintf( pbuf, "Randomize exits 0 to %d &p->&w %s (&R%d&w)&D\n\r", pReset->arg2, rname, pReset->arg1 );
            break;
      }
...
void do_fixmobs( CHAR_DATA * ch, char *argument )
{
   CHAR_DATA *mob;
   OBJ_DATA *obj;
   OBJ_DATA *obj_next;
   ...
   for( mob = last_char; mob != NULL; mob = gch_prev )
   {
      if( !IS_NPC( mob ) )
         continue;

      for( obj = mob->first_carrying; obj != NULL; obj = obj_next )
      {
         obj_next = obj->next_content;
         remove_obj( mob, obj->wear_loc, TRUE );
      }

      for( obj2 = mob->first_carrying; obj2 != NULL; obj2 = obj2_next )
      {
         obj2_next = obj2->next_content;
         wear_obj( mob, obj2, FALSE, -1, TRUE );
      }
   }
   send_to_char( "Mobs fixed\n\r", ch );
}

void do_instaworld( CHAR_DATA * ch, char *argument )
{
   AREA_DATA *pArea;
   int vnum;
   ROOM_INDEX_DATA *pRoom;
