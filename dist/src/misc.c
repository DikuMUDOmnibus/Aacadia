...
void do_drink( CHAR_DATA * ch, char *argument )
{
   ...
   switch ( obj->item_type )
   {
      ...
      case ITEM_BLOOD:
         ...
         {
            if( obj->timer > 0 && ch->level > 5 && ch->blood > ( ch->max_blood - ( ch->max_blood / 10 ) ) )
            {
      ...
      case ITEM_FOUNTAIN:
         ...
         if( obj->value[1] )
         {
            ...
            act( AT_MAGIC, "$n is blind!", ch, NULL, NULL, TO_ROOM );
            act( AT_MAGIC, "You are blind!", ch, NULL, NULL, TO_CHAR );
            af.type = gsn_blindness;
            af.location = APPLY_HITROLL;
            af.modifier = -4;
            af.duration = ( 1 + ( ch->level / 3 ) ) * obj->value[1];
         ...
         if( obj->value[2] )
         {
            ...
            act( AT_RED, "$n feels different as $e changes to the opposite sex!", ch, NULL, NULL, TO_ROOM );
            act( AT_RED, "You feel different as you change to the opposite sex!", ch, NULL, NULL, TO_CHAR );
            af.type = gsn_change_sex;
            af.duration = 10 * ch->level * ( obj->value[2] / 2 );
            af.location = APPLY_SEX;
            do
            {
               af.modifier = number_range( 0, 1 ) - ch->sex;
            }
            while( af.modifier == 0 );
         ...
         if( obj->value[4] )
         {
            ...
            act( AT_MAGIC, "$n cursed $m!", ch, NULL, NULL, TO_ROOM );
            act( AT_MAGIC, "You curse your being!", ch, NULL, NULL, TO_CHAR );
            af.type = gsn_curse;
            af.duration = 3 * obj->value[4];
            af.location = APPLY_HITROLL;
            af.modifier = -10;
         ...
         if( obj->value[5] )
         {
            ...
            update_pos( ch );
            send_to_char( "OH NO! You hurt really bad!", ch );
         }
         break;
   ...
   for( obj = ch->first_carrying; obj; obj = obj_next )
   {
      ...
      if( ( IS_OBJ_STAT( obj, ITEM_ANTI_MALE ) && ch->sex == 0 )
...
void do_rapon( CHAR_DATA * ch, char *argument )
{
   EXIT_DATA *pexit;
   char arg[MAX_INPUT_LENGTH];
...
void do_delete( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg[0] == '\0' )
   {
      send_to_char( "If you would like to delete this character, the syntax is:\n\r", ch );
      send_to_char( "\tdelete <password> <yes/no>\n\r", ch );
      return;
   }
   ...
   if( !strcmp( arg2, "yes" ) )
   {
      if( ch->level >= 100 )
      {
         send_to_char( "I'm sorry. Heroes and up cannot delete.\n\r", ch );
         return;
      }
      do_destroy( ch, ch->name );
      sprintf( buf, "%s has deleted his character.", ch->name );
      log_string( buf );
      return;
   }
   else
