...
/************************************************************************/
/*				 Special Function Handling module	                    */
/************************************************************************/
...
SPEC_FUN *spec_lookup( const char *name )
{
   ...
   if( !str_cmp( name, "spec_healer" ) )
      return spec_healer;
   if( !str_cmp( name, "spec_crier" ) )
      return spec_crier;
   return 0;
}
...
char *lookup_spec( SPEC_FUN * special )
{
   ...
   if( special == spec_questmaster )
      return "spec_questmaster";
   if( special == spec_healer )
      return "spec_healer";
   if( special == spec_crier )
      return "spec_crier";
   return "";
}
...
void summon_if_hating( CHAR_DATA * ch )
{
   ...
   if( ch->fighting || ch->fearing
       || !ch->hating || xIS_SET( ch->in_room->room_flags, ROOM_SAFE ) || xIS_SET( ch->in_room->room_flags, ROOM_SAFETY ) )
      return;
   if( ch->hunting )
      return;
...
bool spec_healer( CHAR_DATA * ch )
{
...

   {
      ...
      if( ( victim != ch && can_see( ch, victim ) && number_bits( 1 ) == 0 )
          && !IS_IMMORTAL( victim ) && !IS_NPC( victim ) && ch->position == 12 )
         break;
   }
   if( !victim )
      return FALSE;
...

   {
      ...
      case 4:
         ...
         spell_acadia( skill_lookup( "refresh" ), ch->level, ch, victim );
         return TRUE;
      case 5:
         ...
         return TRUE;
      case 6:
         act( AT_MAGIC, "$n utters the word 'eklai'.", ch, NULL, NULL, TO_ROOM );
         spell_heal_i( skill_lookup( "heal" ), ch->level, ch, victim );
         return TRUE;
      case 7:
         act( AT_MAGIC, "$n utters the word 'eklahai'.", ch, NULL, NULL, TO_ROOM );
         spell_heal_ii( skill_lookup( "heal ii" ), ch->level, ch, victim );
         return TRUE;
   }
   return FALSE;
}
...
bool spec_cast_adept( CHAR_DATA * ch )
{
...

   {
      case 0:
         act( AT_MAGIC, "$n utters the word 'ciroht'.", ch, NULL, NULL, TO_ROOM );
         spell_acadia( skill_lookup( "armor" ), ch->level, ch, victim );
         return TRUE;
      case 1:
         act( AT_MAGIC, "$n utters the word 'sunimod'.", ch, NULL, NULL, TO_ROOM );
         spell_acadia( skill_lookup( "bless" ), ch->level, ch, victim );
         return TRUE;
      case 2:
      ...
      case 3:
         act( AT_MAGIC, "$n utters the word 'nran'.", ch, NULL, NULL, TO_ROOM );
         spell_acadia( skill_lookup( "cure light" ), ch->level, ch, victim );
         return TRUE;
      case 4:
         act( AT_MAGIC, "$n utters the word 'nyrcs'.", ch, NULL, NULL, TO_ROOM );
         spell_cure_poison( skill_lookup( "cure poison" ), ch->level, ch, victim );
      ...
      case 6:
         act( AT_MAGIC, "$n utters the word 'naimad'.", ch, NULL, NULL, TO_ROOM );
         spell_acadia( skill_lookup( "cure serious" ), ch->level, ch, victim );
         return TRUE;
      case 7:
...
bool spec_guard( CHAR_DATA * ch )
{
   ...
   if( victim && ( xIS_SET( ch->in_room->room_flags, ROOM_SAFE ) || xIS_SET( ch->in_room->room_flags, ROOM_SAFETY ) ) )
...
bool spec_thief( CHAR_DATA * ch )
{
...

   {
      ...
      {
         act( AT_ACTION, "You discover $n's hands in your sack of katyr!", ch, NULL, victim, TO_VICT );
         act( AT_ACTION, "$N discovers $n's hands in $S sack of katyr!", ch, NULL, victim, TO_NOTVICT );
         return TRUE;
      }
