...
#define MAX_COLOR 16
   ...
   if( level >= LEVEL_HEADBUILD && ( !str_cmp( arg, "def" ) || !str_cmp( arg, "default" ) ) )
   {
      def = TRUE;
      argument = one_argument( argument, arg );
      victim = NULL;
   }
   else if( level >= LEVEL_HEADBUILD && ( victim = get_char_room( ch, arg ) ) != NULL )
   {
   ...
   if( !*arg )
   {
      for( at = 0; at < AT_MAXCOLOR; ++at )
      {
         ...
         if( color >= AT_BLINK )
         {
            ...
            blink = FALSE;
         if( at_color_table[at].name && colors[color] && color_names[color] )
