...
//    WAIT_STATE( ch, PULSE_VIOLENCE );
   sprintf( log_buf, "$n draws %s from $p.", arrow->short_descr );
   act( AT_ACTION, log_buf, ch, quiver, NULL, TO_ROOM );
   sprintf( log_buf, "You draw %s from $p.", arrow->short_descr );
    ...
    */
   wear_obj( ch, arrow, TRUE, -1, FALSE );
   return;
}
...
ch_ret projectile_hit( CHAR_DATA * ch, CHAR_DATA * victim, OBJ_DATA * wield, OBJ_DATA * projectile, sh_int dist )
{
   ...
   if( diceroll == 0 || ( diceroll != 19 && diceroll < thac0 - victim_ac ) )
   {
      ...
      {
         ch->carry_number -= get_obj_number( projectile );
         extract_obj( projectile );
      }
      else
      {
   ...
   if( projectile->value[3] == 9 )
   {
      dam = .8 * dam;
      if( ch->race == 1 || ch->race == 9 || ch->race == 11 || ch->race == 33 ||
          ch->race == 19 || ch->race == 23 || ch->race == 24 || ch->race == 25 )
      {
         dam = 1.1 * dam;
      }
   }
   if( projectile->value[3] == 12 )
   {
      dam = .9 * dam;
      if( ch->race == 1 || ch->race == 9 || ch->race == 11 || ch->race == 33 ||
          ch->race == 19 || ch->race == 23 || ch->race == 24 || ch->race == 25 )
   ...
   if( projectile->value[3] == 10 )
   {
      ...
      if( number_range( 1, 100 ) > 75 )
      {
         ...
         act( AT_MAGIC, "$n forms a dark field upon you.", ch, NULL, victim, TO_VICT );
      }
   }
      ...
      else
      {
         ...
         switch ( pchance )
         {
            ...
            case 3: /* Hit in the arm */
               ...
               wear_obj( victim, projectile, TRUE, get_wflag( "lodge_arm" ), FALSE );
               break;
            case 4:
            case 5:
            case 6: /* Hit in the leg */
               SET_BIT( projectile->wear_flags, ITEM_LODGE_LEG );
               wear_obj( victim, projectile, TRUE, get_wflag( "lodge_leg" ), FALSE );
               break;
            case 7:
            case 8:
            case 9:
            case 10:   /* Hit in the chest */
               ...
               wear_obj( victim, projectile, TRUE, get_wflag( "lodge_rib" ), FALSE );
               break;
         }
      }
      return retcode;
 ...
 */
ch_ret ranged_got_target( CHAR_DATA * ch, CHAR_DATA * victim, OBJ_DATA * weapon, OBJ_DATA * projectile, sh_int dist,
...
void do_fire( CHAR_DATA * ch, char *argument )
{
   ...
   if( ch->class != CLASS_ARCHER && !IS_IMMORTAL( ch ) )
   {
      send_to_char( "You are not quite sure how to do this.\n\r", ch );
      return;
   }
