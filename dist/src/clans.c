...
void save_clan( CLAN_DATA * clan )
{
   ...
   else
   {
      ...
      fprintf( fp, "Badge        %s~\n", clan->badge );
      fprintf( fp, "Memrank     %d\n", clan->memrank );
      fprintf( fp, "Leadrank     %d\n", clan->leadrank );
      fprintf( fp, "Onerank      %d\n", clan->onerank );
      fprintf( fp, "Tworank      %d\n", clan->tworank );
   ...
   for( ;; )
   {
      ...
      switch ( UPPER( word[0] ) )
      {
         ...
         case 'L':
            ...
            KEY( "Leadrank", clan->leadrank, fread_number( fp ) );
            KEY( "LName", clan->longname, fread_string( fp ) );
            break;
         ...
         case 'M':
            ...
            KEY( "Memrank", clan->memrank, fread_number( fp ) );
         ...
         case 'T':
            KEY( "Tworank", clan->tworank, fread_number( fp ) );
...
void do_induct( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg[0] == '\0' || arg2[0] == '\0' )
   {
      send_to_char( "Syntax: induct <player> <member/one/two/leader>\n\r", ch );
      return;
   }
...
/*
    ...
    &&   clan->clan_type != CLAN_GUILD && clan->pkyorn == 1 )
...
*/
   ...
   if( !str_cmp( arg2, "leader" ) && clan->pkyorn == 1 )
   {
      clan->members++;
      if( clan->clan_type != CLAN_ORDER && clan->clan_type != CLAN_GUILD )
         SET_BIT( victim->speaks, LANG_CLAN );
...
void do_coe( CHAR_DATA * ch, char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;

   if( IS_NPC( ch ) || ch->pcdata->clan_c == 0 )
   {
      send_to_char( "Huh?\n\r", ch );
      return;
   }
   ...
   if( arg[0] == '\0' )
   {
      send_to_char( "Induct whom into CoE?\n\r", ch );
      return;
   }
...
void do_setclan( CHAR_DATA * ch, char *argument )
{
   ...
   if( arg1[0] == '\0' )
   {
      ...
      send_to_char( " leadrank onerank tworank memrank\n\r", ch );
      send_to_char( " obj1 obj2 obj3 obj4 obj5\n\r", ch );
      if( get_trust( ch ) >= LEVEL_BUILD )
      {
         send_to_char( " name longname filename motto desc\n\r", ch );
         send_to_char( " favour strikes type class pkyorn\n\r", ch );
      }
      if( get_trust( ch ) >= LEVEL_BUILD )
   ...
   {
      clan->leadrank = 0;
      clan->leadrank = atoi( argument );
      send_to_char( "Done.\n\r", ch );
      save_clan( clan );
      return;
   }
   if( !str_cmp( arg2, "memrank" ) )
   {
      clan->memrank = 0;
      clan->memrank = atoi( argument );
      send_to_char( "Done.\n\r", ch );
      save_clan( clan );
      return;
   }
   ...
   {
      clan->tworank = 0;
      clan->tworank = atoi( argument );
      send_to_char( "Done.\n\r", ch );
      save_clan( clan );
      return;
   }
   if( !str_cmp( arg2, "pkyorn" ) )
   {
      clan->pkyorn = 0;
      clan->pkyorn = atoi( argument );
      send_to_char( "Done.\n\r", ch );
      save_clan( clan );
      return;
   }
   ...
   if( !str_cmp( arg2, "longname" ) )
   {
      STRFREE( clan->longname );
      clan->longname = STRALLOC( argument );
      send_to_char( "Done.\n\r", ch );
      save_clan( clan );
      return;
   }
...
void do_showclan( CHAR_DATA * ch, char *argument )
{
   ...
   ch_printf_color( ch,
                    ...
                    clan->name, clan->badge ? clan->badge : "(not set)", clan->longname, clan->filename, clan->motto );
   ch_printf_color( ch, "&wMember Rank: &W%d\t\t&wPK Yes or No: &W%s\n\r", clan->memrank, clan->pkyorn == 1 ? "Yes" : "No" );
   ch_printf_color( ch, "&wDesc     : &W%s\n\r&wDeity    : &W%s\n\r", clan->description, clan->deity );
   ch_printf_color( ch, "&wLeader   : &W%-19.19s\t&wRank: &W%d\n\r", clan->leader, clan->leadrank );
   ch_printf_color( ch, "&wNumber1  : &W%-19.19s\t&wRank: &W%d\n\r", clan->number1, clan->onerank );
   ch_printf_color( ch, "&wNumber2  : &W%-19.19s\t&wRank: &W%d\n\r", clan->number2, clan->tworank );
   ch_printf_color( ch,
...
void do_clans( CHAR_DATA * ch, char *argument )
{
   ...
   if( argument[0] == '\0' )
   {
      ...
      for( clan = first_clan; clan; clan = clan->next )
      {
         ...
         ch_printf( ch, "%-5s %-25.25s %-20.20s %-20.20s\n\r", clan->name, clan->longname, clan->deity, clan->leader );
         count++;
      }
      set_char_color( AT_RED, ch );
      if( !count )
   ...
   set_char_color( AT_RED, ch );
   ch_printf( ch, "\n\r%s, %s\n\r'%s'\n\r\n\r", clan->name, clan->longname, clan->motto );
   set_char_color( AT_GREY, ch );
   ...
   if( clan->pkyorn == 1 )
   {
      ...
      ch_printf_color( ch, "    &wTotal...  &r%-4d\n\r",
                       ( clan->pkills[2] + clan->pkills[3] + clan->pkills[4] + clan->pkills[5] + clan->pkills[6] ) );
   }
   else
   {
      ch_printf_color( ch, "    &wThis clan is non-pk.\n\r" );
   }
   set_char_color( AT_GREY, ch );
...
void do_shove( CHAR_DATA * ch, char *argument )
{
   ...
   if( ch->class == CLASS_THIEF )
      chance = 30;
   if( ch->class == CLASS_MAGE || ch->class == CLASS_TAEDIEN || ch->class == CLASS_BARD || ch->class == CLASS_MONK )
      chance = 15;
...
void do_set( CHAR_DATA * ch, char *argument )
{
   ...
   if( clan->leader )
      leader = capitalize( clan->leader );
   if( clan->number1 )
      number1 = capitalize( clan->number1 );
   if( clan->number2 )
      number2 = capitalize( clan->number2 );
   if( clan->deity )
      deity = capitalize( clan->deity );
   ...
   if( str_cmp( ch->name, clan->leader ) )
   {
      send_to_char( "You're not the leader!!\n\r", ch );
      return;
   }

   if( arg1[0] == '\0' || arg2[0] == '\0' )
   {
      send_to_char( "Syntax: clset <field> <choice|none>\n\r", ch );
      send_to_char( "\n\r", ch );
      send_to_char( "  Fields:  number1 number2 deity badge\n\r", ch );
      return;
   }

   if( !str_cmp( arg1, "number1" ) )
   {
      if( !str_cmp( ( arg2 ), ( deity ) ) || !str_cmp( ( arg2 ), ( number2 ) ) || !str_cmp( ( arg2 ), ( leader ) ) )
      {
         ch_printf( ch, "%s already holds a position, they don't need another!!!!\n\r", capitalize( arg2 ) );
         return;
      }
      ...
      {
         clan->number1 = STRALLOC( capitalize( arg2 ) );
      }
      send_to_char( "Done.\n\r", ch );
      save_clan( clan );
      return;
   ...
   {
      if( !str_cmp( ( arg2 ), ( deity ) ) || !str_cmp( ( arg2 ), ( number1 ) ) || !str_cmp( ( arg2 ), ( leader ) ) )
      {
         ch_printf( ch, "%s already holds a position, they don't need another!!!!\n\r", capitalize( arg2 ) );
         return;
      }
      ...
      {
         clan->number2 = STRALLOC( capitalize( arg2 ) );
      }
      send_to_char( "Done.\n\r", ch );
      save_clan( clan );
      return;
   ...
   {
      if( !str_cmp( ( arg2 ), ( leader ) ) || !str_cmp( ( arg2 ), ( number1 ) ) || !str_cmp( ( arg2 ), ( number2 ) ) )
      {
         ch_printf( ch, "%s already holds a position, they don't need another!!!!\n\r", capitalize( arg2 ) );
         return;
      }
      ...
      {
         clan->deity = STRALLOC( capitalize( arg2 ) );
      }
      send_to_char( "Done.\n\r", ch );
      save_clan( clan );
      return;
   ...
   ch_printf( ch, "Invalid argument '%s'\n\r", arg1 );
   return;
}

void do_crank( CHAR_DATA * ch, char *argument )
{
   char arg[MAX_INPUT_LENGTH];
   char arg2[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;
   ...
   if( ( ch->pcdata && ch->pcdata->bestowments
         && is_name( "crank", ch->pcdata->bestowments ) )
       || !str_cmp( ch->name, clan->deity )
       || !str_cmp( ch->name, clan->leader ) || !str_cmp( ch->name, clan->number1 ) || !str_cmp( ch->name, clan->number2 ) )
      ;
   else
   ...
   if( arg[0] == '\0' || arg2[0] == '\0' )
   {
      send_to_char( "Syntax: crank <player> <member/one/two/leader>\n\r", ch );
      return;
   }
   ...
   if( !victim->pcdata->clan )
   {
      send_to_char( "This player doesn't belong to your clan!\n\r", ch );
      return;
   }
   ...
   {
      victim->pcdata->rank = clan->memrank;
      if( !str_cmp( clan->leader, victim->name ) )
      {
         STRFREE( clan->leader );
         clan->leader = STRALLOC( "" );
      }
      if( !str_cmp( clan->number1, victim->name ) )
      {
         STRFREE( clan->number1 );
         clan->number1 = STRALLOC( "" );
      }
      if( !str_cmp( clan->number2, victim->name ) )
      {
         STRFREE( clan->number2 );
         clan->number2 = STRALLOC( "" );
      }
