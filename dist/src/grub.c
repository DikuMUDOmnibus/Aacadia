...
void do_opfind( CHAR_DATA * ch, char *argument )
{
   OBJ_INDEX_DATA *pObj;
   MPROG_DATA *pProg;
...
void do_rpfind( CHAR_DATA * ch, char *argument )
{
...
#define  MAX_DISPLAY_LINES  14000
...
#define RGRUB_ST_MAX_SIZE 5000
      ...
      qsort( vnum, hit_cou, sizeof( int ), rgrub_int_comp );
...
bool go_parse_operator( CHAR_DATA * ch, char *pch, int *op_num )
{
   ...
   if( go_fd[go_op[*op_num].field].num )
   {
      ...
      if( isdigit( *pch ) )
         ...
         go_op[*op_num].nval = get_otype( pch );
      else if( go_op[*op_num].field == OWEAR )
         go_op[*op_num].nval = owear_to_num( pch );
   }
   else
   {
              ...
              bool sor_dir, bool or_sw, bool np_sw, bool nm_sw, bool ng_sw, bool d2_sw )
{
   ...
   AFFECT_DATA *pa;
   GO_STRUCT r;
   GO_STRUCT a[MAX_DISPLAY_LINES];
   GO_STRUCT *p[MAX_DISPLAY_LINES];
   bool ok_otype[255];
   int tot_match = 0;
   bool res;
   int ind;
...
void do_ogrub( CHAR_DATA * ch, char *argument )
{
   ...
   if( !go_read( ch, dis_num, op_num, sor_ind, sor_dir, or_sw, np_sw, nm_sw, ng_sw, d2_sw ) )
      return;
}
...
void gr_init( void )
{
   ...
   gr_fd[6].num = TRUE;
   strcpy( gr_fd[7].nam, "silver" );
   gr_fd[7].num = TRUE;
   strcpy( gr_fd[8].nam, "copper" );
   gr_fd[8].num = TRUE;
   strcpy( gr_fd[9].nam, "clan" );
   gr_fd[9].num = TRUE;
   strcpy( gr_fd[10].nam, "council" );
   gr_fd[10].num = TRUE;
   strcpy( gr_fd[11].nam, "site" );
   gr_fd[11].num = FALSE;
   strcpy( gr_fd[12].nam, "last" );
   gr_fd[12].num = TRUE;
   strcpy( gr_fd[13].nam, "pkill" );
   gr_fd[13].num = FALSE;
}
...
bool gr_parse_operand( CHAR_DATA * ch, char *arg, bool * or_sw, int *op_num )
{
   ...
   for( cou = 1; cou <= GR_NUM_FIELDS; cou++ )
      if( !str_prefix( gr_fd[cou - 1].nam, arg ) )
      {
         arg += strlen( gr_fd[cou - 1].nam );
         gr_op[*op_num].field = cou - 1;
...
void gr_read( CHAR_DATA * ch, int op_num, bool or_sw, int dis_num )
{
   ...
   char race[][3] = { "Hu", "El", "Dw", "Ha", "Px", "Va", "Og", "HO", "HT", "HE", "Gi",
      "Dr", "SE", "Li", "Gn"
   };
   char clan[][4] = {
   ...
   while( !feof( fp ) )
   {
      ...
      {
         tot_match++;
         if( !title_sw && dis_num > 0 )
         {
            ch_printf( ch,
                       "\n\r%-12s %-2s %1s %-2s %1s %3s %3s %5s %11s %11s %11s %-15s %-6s %s\n\r",
                       "Name", "Lv", "S", "R", "C", "Cln", "Cou", "Room", "Katyr", "Siam", "Rona", "Site", "Last", "Pk" );
            title_sw = TRUE;
         }
         if( tot_match <= dis_num )
            ch_printf( ch,
                       ...
                       council[( unsigned char )r.council], r.room, r.gold, r.silver, r.copper, r.site, r.last, r.pkill );
      }
      fread( &r, sizeof( r ), 1, fp );
   }
   fclose( fp );
...
void do_grub( CHAR_DATA * ch, char *argument )
{
   ...
   gr_read( ch, op_num, or_sw, dis_num );
}
...
void diag_ins( OBJ_INDEX_DATA * p, int siz, OBJ_INDEX_DATA ** f, CHAR_DATA * ch )
{
   ...
   if( !f[siz - 1] || p->count > f[siz - 1]->count )
      while( cou < siz && ins < 0 )
