...
ch_ret move_char( CHAR_DATA * ch, EXIT_DATA * pexit, int fall )
{
   ...
   if( !fall
       && ( IS_AFFECTED( ch, AFF_CHARM ) || IS_AFFECTED( ch, AFF_JMT ) ) && ch->master && in_room == ch->master->in_room )
   {
   ...
   if( room_is_dnd( ch, to_room ) || room_is_HAdnd( ch, to_room ) )
   {
   ...
   if( !IS_NPC( ch ) )
   {
      for( mob = ch->in_room->first_person; mob; mob = mob_next )
      {
         ...
         if( xIS_SET( mob->act, ACT_BLOCKER ) )
         {
            sprintf( buf2, "%s It's not that easy to get past me!", ch->name );
...
// WAIT_STATE( ch, move );
   ...
   if( ch->level > 0 )
   {
      ...
      for( vch = from_room->first_person; vch; vch = vch_next )
      {
         vch_next = vch->next_in_room;

         if( ( !IS_AFFECTED( ch, AFF_SNEAK )
               || xIS_SET( vch->affected_by, AFF_ALERTNESS ) ) && ( IS_NPC( ch ) || !xIS_SET( ch->act, PLR_WIZINVIS ) ) )
         {
            if( fall )
               txt = "falls";
            else if( !txt )
            ...
            if( !IS_NPC( ch ) )
            {
               ...
               {
                  act( AT_ACTION, ch->pcdata->moveout, ch, NULL, vch, TO_VICT );
               }
               else
               {
                  if( ch->mount )
                  ...
                  else
                  {
                     ...
                     act( AT_ACTION, buf, ch, dir_name[door], vch, TO_VICT );
                  }
               }
            }
            else
   ...
   if( ch->level > 0 )
   {
      ...
      for( vch = to_room->first_person; vch; vch = vch_next )
      {
         vch_next = vch->next_in_room;
               ...
               || xIS_SET( vch->affected_by, AFF_ALERTNESS ) ) && ( IS_NPC( ch ) || !xIS_SET( ch->act, PLR_WIZINVIS ) ) )
         {
            ...
            if( !IS_NPC( ch ) )
            {
               ...
               {
                  act( AT_ACTION, ch->pcdata->movein, ch, NULL, vch, TO_VICT );
               }
               else
               {
                  if( ch->mount )
   ...
   if( LEARNED( ch, gsn_map_lore ) && ( from_room->area != to_room->area ) )
   {
      sprintf( buf3, "&BYou sense you are now entering the %s...&D\n\r", sec_flags[to_room->sector_type] );
   ...
   if( xIS_SET( ch->in_room->room_flags, ROOM_DEATH ) && !IS_IMMORTAL( ch ) )
   {
      ...
      log_string( buf );
      to_channel( buf, CHANNEL_MONITOR, "Monitor", LEVEL_IMMORTAL, ch->level );
      if( !IS_NPC( vic ) )
      {
         sprintf( buf, "&R<&BDEATH INFO&R> %s has just been killed by %s, in %s (%d).&D", vic->name,
                  ( IS_NPC( ch ) ? ch->short_descr : ch->name ), vic->in_room->name, vic->in_room->vnum );
         talk_info( AT_PLAIN, buf );
      }
      set_cur_char( vic );
      raw_kill( ch, vic, TRUE );
      vic = NULL;
...
void do_north( CHAR_DATA * ch, char *argument )
{
   ...
   for( block = ch->in_room->first_person; block; block = block->next_in_room )
   {
      if( IS_NPC( block ) )
      {
         if( xIS_SET( block->act, ACT_BLOCKER ) )
         {
            sprintf( buf2, "%s It's not that easy to get past me!", ch->name );
            do_say_to_char( block, buf2 );
            return;
         }
      }
   }
   ...
   else
   {
      for( pexit = ch->in_room->first_exit; pexit; pexit = pexit->next )
      {
         if( ( quiet || IS_SET( pexit->exit_info, EX_ISDOOR ) )
             && pexit->keyword && ( nifty_is_name( arg, pexit->keyword ) || nifty_is_name_prefix( arg, pexit->keyword ) ) )
            return pexit;
      }
      if( !quiet )
...
void do_bashdoor( CHAR_DATA * ch, char *argument )
{
   ...
   if( !IS_NPC( ch ) && !DUAL_SKILL( ch, gsn_bashdoor ) )
   {
...
//     WAIT_STATE( ch, skill_table[gsn_bashdoor]->beats );
   ...
   if( !char_died( ch ) )
      for( gch = ch->in_room->first_person; gch; gch = gch->next_in_room )
      {
         if( IS_AWAKE( gch )
             && !gch->fighting
             && ( IS_NPC( gch ) && ( !IS_AFFECTED( gch, AFF_CHARM ) || !IS_AFFECTED( gch, AFF_JMT ) ) )
...
void do_stand( CHAR_DATA * ch, char *argument )
{
   ...
   if( ch->position == POS_FIGHTING
       ...
       || ch->position == POS_EVASIVE || ch->position == POS_DEFENSIVE || ch->position == POS_AGGRESSIVE )
   {
...
void do_sleep( CHAR_DATA * ch, char *argument )
{
   ...
   if( xIS_SET( ch->in_room->room_flags, ROOM_NO_SLEEP ) )
   {
      if( !IS_AGOD( ch ) && IS_SET( ch->in_room->area->flags, AFLAG_SLEEPDAM ) )
      {
         send_to_char( "You are attacked by a mysterious force.\n\r", ch );
         act( AT_ACTION, "$n is attacked by a mysterious force.", ch, NULL, NULL, TO_ROOM );
         ch->hit -= ( ch->level / 4 );
      }
      else
      {
         send_to_char( "You are prevented from sleeping.\n\r", ch );
         act( AT_ACTION, "$n is prevented from sleeping.", ch, NULL, NULL, TO_ROOM );
      }
      return;
   }
