...
char get_map_code( RID * room, int mode )
{
   ...
   switch ( mode )
   {
      ...
      case 10:
         ...
         return ( char )'O';
      case 9:
         if( xIS_SET( room->room_flags, ROOM_SAFE ) || xIS_SET( room->room_flags, ROOM_SAFETY ) )
            return ( char )'X';
         return ( char )'O';
      case 8:
...
void do_mapout( CD * ch, char *argument )
{
   ...
   MID *map_index;
   MD *map, *tmp;
   RID *this_rm, *tmp_r = NULL;
   XD *tmp_x;
   char code;
   int rooms, rows, cols, row, col, mapnum, x, y, avail_rooms;
